...
function Admin() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(donorsRef, (snapshot) => {
      ...
      const donorList = data ? Object.entries(data).map(([key, value]) => ({ ...value, _id: key })) : [];
      setDonors(donorList);
    });
    return () => unsubscribe();
  }, []);
  ...
  // fetchDonors removed, now using Firebase listener
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEditing) {
      try {
        const donorRef = ref(database, `donors/${formData._id}`);
        await update(donorRef, formData);
        setIsEditing(false);
        displayMessage("Donor updated successfully!");
      } catch (error) {
        console.error("Error updating donor:", error);
        displayMessage(error.message || "Failed to update donor.", "error");
      }
    } else {
      try {
        const donorsRef = ref(database, "donors");
        await push(donorsRef, formData);
        displayMessage("Donor added successfully!");
      } catch (error) {
        console.error("Error adding donor:", error);
        displayMessage(error.message || "Failed to add donor.", "error");
      }
    }
    setFormData({ _id: null, name: "", email: "", bloodType: "", phone: "", address: "" });
  };
  ...
  const handleEdit = (donor) => {
    setFormData({
      ...
      hasDonatedRecently: donor.hasDonatedRecently === true ? true : false,
    });
    setIsEditing(true);
  };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      displayMessage("Donor deleted successfully!");
    } catch (error) {
      console.error("Error deleting donor:", error);
      displayMessage("Failed to delete donor.", "error");
    }
  };
